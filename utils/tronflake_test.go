...
func init() {
	...
	sf = NewTronflake(st)
	if sf == nil {
		panic("tronflake not created")
	}

	startTime = toTronflakeTime(st.StartTime)
...
func TestTronflakeOnce(t *testing.T) {
	sleepTime := uint64(50)
	...
	fmt.Println("tronflake id:", id)
	fmt.Println("decompose:", parts)
}

func currentTime() int64 {
	return toTronflakeTime(time.Now())
}

func TestTronflakeFor10Sec(t *testing.T) {
	var numID uint32
	var lastID uint64
...
func TestTronflakeInParallel(t *testing.T) {
...
func TestNilTronflake(t *testing.T) {
	var startInFuture Settings
	startInFuture.StartTime = time.Now().Add(time.Duration(1) * time.Minute)
	if NewTronflake(startInFuture) != nil {
		t.Errorf("tronflake starting in the future")
	}
	...
	if NewTronflake(noMachineID) != nil {
		t.Errorf("tronflake with no machine id")
	}
	...
	if NewTronflake(invalidMachineID) != nil {
		t.Errorf("tronflake with invalid machine id")
	}
}

func pseudoSleep(period time.Duration) {
	sf.startTime -= int64(period) / tronflakeTimeUnit
}
